...
def _do_depth_crawl(_dc):
    ...
    while True:
        ...
        try:
            ...
            traceback.print_tb(err.__traceback__)
            logger.exception("ProtocolError -> sleeping{} {}".format(_dc.market, err.__traceback__))
            sleep(60)
...
def _do_volume_crawl(_vc):
    ...
    while True:
        ...
        if len(trades[_vc.market]) > 200:
            logger.info(f"{_vc.market} trading volume list size is greater than {len(trades[_vc.market])}")
        sleep(60*60)
